...
* **修正后的水平角观测，其中 $\theta,\alpha_0$ 分别是定向点的已知方向值，是定向观测值**
...
给定线性方程组 $A \mathbf{X} = b$，其中 $A$ 是一个 $m \times n$ 的矩阵， $\mathbf{X}$ 是一个 $n \times 1$ 的向量， $\mathbf{b}$ 是一个 $m \times 1$ 的向量，协方差矩阵 $P$ 是一个 $m \times m$ 的对称正定矩阵。最小二乘解为：
...
$$
A^T P^{-1} b = A_1^T P_{11}^{-1} b_1 + A_2^T P_{22}^{-1} b_2
$$

5. **最终解**：
   
$$
\mathbf{X} = (A_1^T P_{11}^{-1} A_1 + A_2^T P_{22}^{-1} A_2)^{-1} (A_1^T P_{11}^{-1} b_1 + A_2^T P_{22}^{-1} b_2)
$$
...
$$ A_1^T P_{11}^{-1} b_1 = I^T P_{11}^{-1} b_1 = P_{11}^{-1} b_1 $$

5. **代入简化**：
   
$$
\mathbf{X} = (P_{11}^{-1} + A_2^T P_{22}^{-1} A_2)^{-1} (P_{11}^{-1} b_1 + A_2^T P_{22}^{-1} b_2)
$$
...
2. **计算 $A_2^T P_{22}^{-1} A_2$**：
   
$$ A_2^T P_{22}^{-1} A_2 = I^T P_{22}^{-1} I = P_{22}^{-1} $$

3. **计算 $A_1^T P_{11}^{-1} \mathbf{b}_1$**：
   
$$ A_1^T P_{11}^{-1} b_1 = I^T P_{11}^{-1} b_1 = P_{11}^{-1} b_1 $$

4. **计算 $A_2^T P_{22}^{-1} \mathbf{b}_2$**：
   
$$ A_2^T P_{22}^{-1} b_2 = I^T P_{22}^{-1} b_2 = P_{22}^{-1} b_2 $$

5. **代入简化**：

$$ \mathbf{X} = (P_{11}^{-1} + P_{22}^{-1})^{-1} (P_{11}^{-1} b_1 + P_{22}^{-1} b_2) $$
